...
const configSchema = z.object({
  ...
  providers: z.object({
    ...
    asr: z.enum(["fake", "faster-whisper"]),
    asrModel: z.string(),
...
export function loadRuntimeConfig(env: NodeJS.ProcessEnv = process.env, cwd = process.cwd()): RuntimeConfig {
  ...
  return configSchema.parse({
    ...
    providers: {
      ...
      asr: env.VIDEO_AGENT_ASR === "faster-whisper" ? "faster-whisper" : providerFile.asr ?? "fake",
      asrModel: env.VIDEO_AGENT_ASR_MODEL ?? providerFile.asrModel ?? "small",
