...
function finiteTimestamp(value: number, label: string) {
  if (!Number.isFinite(value) || value < 0) throw new Error(`OpenAI transcription returned invalid ${label}`);
  return value;
}
...
function officialOpenAIBase(baseUrl: string) {
  const normalized = baseUrl.replace(/\/+$/u, "");
  if (normalized !== OPENAI_BASE_URL) throw new Error("Mobile media upload is pinned to https://api.openai.com/v1; arbitrary file-upload endpoints are intentionally not supported");
  return normalized;
}
...
function normalizeTranscription(model: MobileOpenAIASRModel, rawText: string, options: { language?: string; prompt?: string }): ASRResult {
  ...
  if (model === "gpt-4o-transcribe-diarize") {
    ...
    const segments: ASRSegmentResult[] = (body.segments ?? []).map((segment) => ({
      text: segment.text.trim(), startSeconds: finiteTimestamp(segment.start, "segment start"), endSeconds: finiteTimestamp(segment.end, "segment end"), speaker: segment.speaker, words: [],
  ...
  let wordCursor = 0;
  const segments: ASRSegmentResult[] = [];
  for (const source of body.segments ?? []) {
    const startSeconds = finiteTimestamp(source.start, "segment start");
    const endSeconds = finiteTimestamp(source.end, "segment end");
    ...
    for (let index = wordCursor; index < words.length && words[index]!.startSeconds < endSeconds + 0.001; index += 1) {
      if (words[index]!.endSeconds > startSeconds - 0.001) segmentWords.push(words[index]!);
    }
...
export class MobileOpenAIASRProvider implements ASRProvider {
  ...
  capabilities(): ASRCapabilities {
    return this.model === "gpt-4o-transcribe-diarize"
      ? { wordTimestamps: false, segmentTimestamps: true, speakerDiarization: true, languageDetection: false, streaming: false, confidence: false, forcedAlignment: false }
      : { wordTimestamps: true, segmentTimestamps: true, speakerDiarization: false, languageDetection: true, streaming: false, confidence: true, forcedAlignment: false };
  }
  ...
  async transcribe(inputPath: string, options: { language?: string; prompt?: string } = {}, context?: OperationContext): Promise<ASRResult> {
    ...
    try {
      ...
      if (context?.signal?.aborted) throw cancellation(context.signal);
      context?.onProgress?.(0.92, "normalizing", "Normalizing speech timestamps");
      const result = normalizeTranscription(this.model, raw, options);
  ...
  async health(): Promise<ProviderHealth> {
    ...
    try {
      const response = await this.http.request({ method: "GET", url: `${OPENAI_BASE_URL}/models/${encodeURIComponent(this.model)}`, headers: { authorization: `Bearer ${this.apiKey}` }, timeoutMs: 5_000 });
      return { id: this.id, status: response.status >= 200 && response.status < 300 ? "ready" : "degraded", message: response.status >= 200 && response.status < 300 ? `${this.model} is reachable` : `Model check returned ${response.status}`, capabilities: { ...this.capabilities() } };
...
function wavDuration(bytes: Uint8Array): { durationSeconds: number; sampleRate: number } {
  ...
  let offset = 12; let channels = 0; let sampleRate = 0; let bitsPerSample = 0; let dataBytes = 0;
  while (offset + 8 <= bytes.byteLength) {
    const id = readAscii(bytes, offset, 4); const size = view.getUint32(offset + 4, true); const body = offset + 8;
    if (body + size > bytes.byteLength) break;
    if (id === "fmt " && size >= 16) { channels = view.getUint16(body + 2, true); sampleRate = view.getUint32(body + 4, true); bitsPerSample = view.getUint16(body + 14, true); }
...
export class MobileOpenAITTSProvider implements TTSProvider {
  ...
  async listVoices(): Promise<VoiceProfile[]> {
    return OPENAI_VOICES.map((voice) => ({ id: `openai-${voice}`, type: "preset", provider: this.id, providerVoiceId: voice, model: this.model, name: voice, languages: ["multilingual"], cloning: false, status: "active", referenceAssetIds: [], authorizationStatus: "not_required", createdAt: new Date(0).toISOString(), usageRestrictions: ["Disclose generated audio as AI-generated", "Availability and use remain subject to provider terms"], providerMetadata: {} }));
  }
  async synthesize(input: { text: string; voiceId: string; language: string; speed?: number }, context?: OperationContext): Promise<TTSResult> {
    ...
    if (response.status < 200 || response.status >= 300) throw new Error(`OpenAI TTS failed (${response.status}): ${new TextDecoder().decode(response.body.slice(0, 1_000))}`);
    if (response.body.byteLength > MAX_TTS_RESPONSE_BYTES) throw new Error(`TTS response exceeds ${MAX_TTS_RESPONSE_BYTES} bytes; use shorter narration chunks`);
    if (context?.signal?.aborted) throw cancellation(context.signal);
    const measured = wavDuration(response.body);
    context?.onProgress?.(1, "tts-complete", "Narration generated");
    return { audio: response.body, format: "wav", durationSeconds: measured.durationSeconds, sampleRate: measured.sampleRate, wordTimings: [], model: this.model, voiceId: input.voiceId };
  }
  async health(): Promise<ProviderHealth> {
    if (!this.apiKey) return { id: this.id, status: "unavailable", message: "OpenAI API key is not configured", capabilities: { ...this.capabilities() } };
    try {
      const response = await this.http.request({ method: "GET", url: `${this.baseUrl}/models/${encodeURIComponent(this.model)}`, headers: { authorization: `Bearer ${this.apiKey}` }, timeoutMs: 5_000 });
      return { id: this.id, status: response.status >= 200 && response.status < 300 ? "ready" : "degraded", message: response.status >= 200 && response.status < 300 ? `${this.model} is reachable` : `Model check returned ${response.status}`, capabilities: { ...this.capabilities() } };
    } catch (error) { return { id: this.id, status: "unavailable", message: error instanceof Error ? error.message : String(error), capabilities: { ...this.capabilities() } }; }
  }
}
...
export class MutableASRProvider implements ASRProvider {
  readonly id = "mobile-configurable-asr"; get model() { return this.current?.model ?? "unconfigured"; }
  private current?: ASRProvider;
  set(provider: ASRProvider) { this.current = provider; }
  configured() { return this.current !== undefined; }
  private provider() { if (!this.current) throw new Error("Configure an ASR provider before transcription"); return this.current; }
...
export class MutableTTSProvider implements TTSProvider {
  readonly id = "mobile-configurable-tts"; get model() { return this.current?.model ?? "unconfigured"; }
