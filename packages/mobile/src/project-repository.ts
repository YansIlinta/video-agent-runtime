...
export class MobileProjectRepository implements ProjectRepository {
  ...
  private async writeJson<T>(projectId: string, relativePath: string, value: T, schema: Schema<T>) { await this.filesystem.write(this.uri(projectId, relativePath), encoder.encode(`${JSON.stringify(schema.parse(value), null, 2)}\n`), { atomic: true }); }
  ...
  private async jsonItems<T>(projectId: string, directory: string, schema: Schema<T>, include: (name: string) => boolean = () => true) { const prefix = this.uri(projectId, directory); let uris: LogicalUri[]; try { uris = await this.filesystem.list(prefix); } catch { return []; } const values = await Promise.all(uris.filter((uri) => uri.endsWith(".json") && include(uri.slice(uri.lastIndexOf("/") + 1))).map(async (uri) => schema.parse(JSON.parse(decoder.decode(await this.filesystem.read(uri)))))); return values; }
  ...
  async projectDiskUsage(projectId: string) { const files = await this.filesystem.list(this.uri(projectId)); const stats = await Promise.all(files.map((uri) => this.filesystem.stat(uri).catch(() => undefined))); return stats.reduce((sum, item) => sum + (item?.kind === "file" ? item.sizeBytes : 0), 0); }
  async prunePreviews(projectId: string, retain: number) { const files = (await this.filesystem.list(this.uri(projectId, "previews"))).filter((uri) => uri.endsWith(".mp4")); const entries = await Promise.all(files.map(async (uri) => ({ uri, stat: await this.filesystem.stat(uri) }))); const removed = entries.sort((a, b) => (b.stat.modifiedAt ?? "").localeCompare(a.stat.modifiedAt ?? "")).slice(Math.max(0, retain)); await Promise.all(removed.map(({ uri }) => this.filesystem.delete(uri))); return removed.map(({ uri }) => uri.slice(uri.lastIndexOf("/") + 1)); }
  async sweepTemporaryFiles(projectId: string) { const files = await this.filesystem.list(this.uri(projectId)); const temporary = files.filter((uri) => /(?:\.tmp|\.captions\.srt)$/u.test(uri) || uri.includes(".tmp.mp4")); await Promise.all(temporary.map((uri) => this.filesystem.delete(uri).catch(() => undefined))); return temporary; }
  ...
  async findTranscriptByCacheKey(id: string, key: string) { return (await this.jsonItems(id, "transcripts", transcriptSchema)).find((item) => item.cacheKey === key); }
  ...
  async findSpeechAssetByCacheKey(id: string, key: string) { return (await this.listSpeechAssets(id)).find((item) => item.cacheKey === key); }
  ...
  async findVoiceReferenceQualityByCacheKey(id: string, key: string) { return (await this.jsonItems(id, "analysis/voice", voiceReferenceQualityReportSchema)).find((item) => item.cacheKey === key); }
