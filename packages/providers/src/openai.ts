...
function compactTranscript(transcript: Transcript): string {
  return JSON.stringify({ language: transcript.language, quality: transcript.quality, segments: transcript.segments.map((segment) => ({ id: segment.id, startUs: segment.startUs, endUs: segment.endUs, speakerId: segment.speakerId, text: segment.normalizedText })) });
}
...
export class OpenAILLMProvider implements LLMProvider {
  ...
  async generateStructured<T>(request: StructuredGenerationRequest<T>): Promise<StructuredGenerationResult<T>> {
    ...
    try {
      ...
      for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
        ...
        const body = JSON.parse(new TextDecoder().decode(response.body)) as ResponsesBody;
      ...
      this.calls.push({ id: this.idValue(), ...(request.projectId ? { projectId: request.projectId } : {}), operation: request.operation, provider: this.id, model: this.model, ...(providerRequestId ? { requestId: providerRequestId } : {}), latencyMs: this.primitives.clock.now().getTime() - started, ...(usage ? { usage } : {}), retryCount, validation: { valid: false, issues: validationIssues }, status: cancelled ? "cancelled" : "failed", createdAt: this.now() });
      throw error;
    } finally {
  ...
  async proposeStrategy(input: { projectId: string; prompt: string; transcript: Transcript; targetDurationUs: number }, context?: OperationContext): Promise<EditingStrategy> {
    ...
    const result = await this.generateStructured<EditingStrategy>({ requestId, projectId: input.projectId, operation: "strategy", schemaName: "editing_strategy", schema: editingStrategySchema as unknown as ZodType<EditingStrategy>, jsonSchema: zodToJsonSchema(editingStrategySchema, { $refStrategy: "none" }) as Record<string, unknown>, instructions: "You are the Strategy Planner for a transcript-first video editor. Return only the requested strategy. Preserve meaning, cite rationale from transcript evidence, and use integer microseconds.", input: JSON.stringify({ projectId: input.projectId, userIntent: input.prompt, targetDurationUs: input.targetDurationUs, transcript: JSON.parse(compactTranscript(input.transcript)), required: { schemaVersion: 1, status: "proposed", uniqueId: true, createdAt: "ISO-8601 now" } }), ...(context?.signal ? { signal: context.signal } : {}) });
    return result.value;
  }
  ...
  async createEditPlan(input: { projectId: string; strategy: EditingStrategy; transcript: Transcript; assetId: string; basedOnVersion: number }, context?: OperationContext): Promise<EditPlan> {
    const result = await this.generateStructured<EditPlan>({ requestId: this.idValue(), projectId: input.projectId, operation: "edit-plan", schemaName: "edit_plan", schema: editPlanSchema as unknown as ZodType<EditPlan>, jsonSchema: zodToJsonSchema(editPlanSchema, { $refStrategy: "none" }) as Record<string, unknown>, instructions: "You are the Edit Planner. Select exact source ranges from transcript segments. Never invent media or wording. Output non-overlapping timeline segments with integer microseconds and a concise reason for every segment.", input: JSON.stringify({ projectId: input.projectId, assetId: input.assetId, basedOnVersion: input.basedOnVersion, strategy: input.strategy, transcript: JSON.parse(compactTranscript(input.transcript)), required: { schemaVersion: 1, strategyId: input.strategy.id, uniqueIds: true, createdAt: "ISO-8601 now" } }), ...(context?.signal ? { signal: context.signal } : {}) });
    return result.value;
  }
  ...
  async createEditPatch(input: { projectId: string; plan: EditPlan; timeline: import("../../core/src/schemas.js").Timeline; transcript: Transcript; feedback: Array<{ id: string; message: string; range?: { startUs: number; endUs: number } }>; basedOnVersion: number }, context?: OperationContext): Promise<EditPatch> {
    const result = await this.generateStructured<EditPatch>({ requestId: this.idValue(), projectId: input.projectId, operation: "patch-plan", schemaName: "edit_patch", schema: editPatchSchema as unknown as ZodType<EditPatch>, jsonSchema: zodToJsonSchema(editPatchSchema, { $refStrategy: "none" }) as Record<string, unknown>, instructions: "You are the Patch Planner. Make the smallest reversible change satisfying feedback. Scope must match the affected range. Do not modify clips outside a local request unless globalChangeJustification explicitly explains why.", input: JSON.stringify({ projectId: input.projectId, basedOnVersion: input.basedOnVersion, plan: input.plan, timeline: input.timeline, feedback: input.feedback, transcript: JSON.parse(compactTranscript(input.transcript)), required: { schemaVersion: 1, uniqueId: true, createdAt: "ISO-8601 now" } }), ...(context?.signal ? { signal: context.signal } : {}) });
    return result.value;
  }

  takeLastCall(projectId?: string): ProviderCall | undefined { const index = projectId ? this.calls.findIndex((call) => call.projectId === projectId) : 0; if (index < 0) return undefined; return this.calls.splice(index, 1)[0]; }
