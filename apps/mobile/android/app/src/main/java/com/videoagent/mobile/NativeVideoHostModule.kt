...
class NativeVideoHostModule(private val context: ReactApplicationContext) : NativeVideoHostSpec(context), ActivityEventListener {
  companion object { const val NAME = "NativeVideoHost"; const val PICK_VIDEO = 9031; const val KEY_ALIAS = "video-agent-byok" }
  ...
  override fun httpJson(requestJson: String, promise: Promise) { Thread { try { val value = JSONObject(requestJson); val connection = URL(value.getString("url")).openConnection() as HttpURLConnection; connection.requestMethod = value.optString("method", "GET"); connection.connectTimeout = value.optInt("timeoutMs", 120000); connection.readTimeout = connection.connectTimeout; value.optJSONObject("headers")?.keys()?.forEach { connection.setRequestProperty(it, value.getJSONObject("headers").getString(it)) }; value.optString("body").takeIf { it.isNotEmpty() }?.let { connection.doOutput = true; connection.outputStream.use { output -> output.write(it.toByteArray()) } }; val status = connection.responseCode; val data = (if (status >= 400) connection.errorStream else connection.inputStream)?.readBytes() ?: ByteArray(0); val headers = JSONObject(); connection.headerFields.filterKeys { it != null }.forEach { (key, values) -> headers.put(key.lowercase(), values.joinToString(",")) }; promise.resolve(JSONObject().put("status", status).put("headers", headers).put("body", JSONArray(data.map { it.toInt() and 255 })).toString()) } catch (error: Throwable) { promise.reject("NETWORK_UNAVAILABLE", error.message, error) } }.start() }

  override fun scheduleBackgroundJson(taskJson: String, promise: Promise) = promise(promise) { val task = JSONObject(taskJson); val request = OneTimeWorkRequestBuilder<VideoAgentWorker>().setInputData(Data.Builder().putString("jobId", task.getString("id")).putString("kind", task.optString("kind")).build()).addTag(task.getString("id")).build(); WorkManager.getInstance(context).enqueue(request); null }
  ...
  override fun pendingBackgroundJson(promise: Promise) = promise(promise) { val values = WorkManager.getInstance(context).getWorkInfosByTag("video-agent").get().filter { !it.state.isFinished }.map { JSONObject().put("id", it.id.toString()).put("kind", "durable-mobile-job") }; JSONArray(values).toString() }
